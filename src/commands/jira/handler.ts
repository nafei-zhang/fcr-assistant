...
import { jiraTools } from '../../services/jira/tools';
...
export async function handleJiraCommand(
    ...
    stream.progress('Analyzing Jira request...');
...
Parameters: ${JSON.stringify(t.parameters)}`;
    ...
    const systemPrompt = `You are an AI assistant for Jira.
        ...
        if (plan.tool) {
            ...
            if (tool) {
                ...
                try {
                    ...
                    if (tool.name === 'jira_search' && result.issues) {
                        stream.markdown('\n**Issues Found:**\n');
                        for (const issue of result.issues) {
                             const key = issue.key;
                             const summary = issue.fields.summary;
                             const status = issue.fields.status.name;
                             const url = issue.self.split('/rest/')[0] + '/browse/' + key; // Construct browse URL
                             stream.markdown(`- [${key}](${url}) ${summary} (${status})\n`);
                        }
                    } else {
